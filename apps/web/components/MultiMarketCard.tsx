    ...
    <div className="bg-gray-800/60 rounded-2xl border border-gray-700/50 overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-purple-900/40 to-pink-900/40 p-4 border-b border-gray-700/50">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm text-gray-400">
            {LEAGUE_FLAGS[league_id] || "🏆"}{" "}
            {LEAGUE_NAMES[league_id] || "League"}
            {is_cup && (
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex flex-col items-center mx-4">
            <span className="text-2xl font-black text-purple-400">
              {expected.home_goals.toFixed(1)} -{" "}
              {expected.away_goals.toFixed(1)}
            </span>
      ...
      <div className="grid grid-cols-3 gap-2 p-3 bg-gray-900/30 border-b border-gray-700/50">
        <div className="text-center">
          <div
            className={`text-lg font-bold ${getProbColor(predictions.over_under.over_under_2_5?.over || 0)}`}
          >
        ...
        <div className="text-center">
          <div
            className={`text-lg font-bold ${getProbColor(predictions.btts.yes)}`}
          >
      ...
      <div className="p-4 min-h-[200px]">
        ...
        {activeTab === "cards" && <CardsTab cards={predictions.cards} />}
        {activeTab === "shots" && (
          <ShotsTab
              ...
              <div key={line.key} className="bg-gray-700/30 rounded-lg p-2">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-green-400">
                    Over: {formatProb(data.over)}
                  </span>
                  <span className="text-red-400">
                    Under: {formatProb(data.under)}
                  </span>
                </div>
              </div>
            );
          ...
          <div className="space-y-1">
            ...
            {["home_over_0_5", "home_over_1_5", "home_over_2_5"].map((key) => {
              ...
              return (
                <div
                  key={key}
                  className="flex justify-between text-xs bg-gray-700/20 rounded px-2 py-1"
                >
                  <span className="text-gray-400">O {data.line}</span>
                  <span className="text-green-400">
                    {formatProb(data.over)}
                  </span>
                </div>
              );
            })}
        ...
        <div className="flex justify-around">
          ...
          <div className="text-center">
            <div className="text-xl font-bold text-red-400">
              {formatProb(predictions.btts.no)}
            </div>
          ...
          {[
            ...
            return (
              <div key={key} className="bg-gray-700/30 rounded-lg p-2">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-green-400">
                    O: {formatProb(data.over)}
                  </span>
                  <span className="text-red-400">
                    U: {formatProb(data.under || 0)}
                  </span>
                </div>
              </div>
            );
...
function CardsTab({
  ...
  cards: MultiMarketPrediction["predictions"]["cards"];
}) {
  const formatProb = (p: number) => `${(p * 100).toFixed(1)}%`;
        ...
        <div className="flex justify-around items-center">
          <div className="text-center">
            <div className="text-2xl font-bold text-yellow-400">
              {expected.home_yellow.toFixed(1)}
            </div>
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-yellow-400">
              {expected.away_yellow.toFixed(1)}
            </div>
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-yellow-500">
              {expected.total_yellow.toFixed(1)}
            </div>
            <div className="text-xs text-gray-500">Total</div>
          </div>
        </div>
            ...
            <div className="flex justify-between items-center">
              <div className="text-center">
                <div className="text-lg font-bold text-green-400">
                  {expected.home_shots_on_target}
                </div>
                <div className="text-xs text-gray-500">{homeTeam}</div>
              </div>
              <div className="text-center">
                <div className="text-lg font-bold text-green-400">
                  {expected.away_shots_on_target}
                </div>
                <div className="text-xs text-gray-500">{awayTeam}</div>
              </div>
            </div>
        ...
        <div className="flex justify-around">
          <div className="text-center">
            <div className="text-xl font-bold text-purple-400">
              {formatProb(halfTime.result_1x2?.home || 0)}
            </div>
            <div className="text-xs text-gray-500">{homeTeam}</div>
          </div>
          <div className="text-center">
            <div className="text-xl font-bold text-gray-400">
              {formatProb(halfTime.result_1x2?.draw || 0)}
            </div>
            <div className="text-xs text-gray-500">Draw</div>
          </div>
          <div className="text-center">
            <div className="text-xl font-bold text-pink-400">
              {formatProb(halfTime.result_1x2?.away || 0)}
            </div>
            <div className="text-xs text-gray-500">{awayTeam}</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {exactScores.slice(0, 8).map((score, idx) => (
            ...
            >
              <div className="flex justify-between items-center">
                <span
                  className={`text-lg font-bold ${idx === 0 ? "text-yellow-400" : "text-white"}`}
                >
                  {score.score}
                </span>
                <span
                  className={`text-sm font-medium ${idx === 0 ? "text-yellow-400" : "text-gray-400"}`}
                >
