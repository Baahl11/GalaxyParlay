...
export function GalaxyCanvas({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const clusters = [
        {
          grade: "A",
          x: centerX - clusterRadius * 1.2,
          y: centerY - clusterRadius * 0.8,
          color: 0x22c55e,
          size: 14,
          glow: 0x22c55e,
        },
        ...
        {
          grade: "C",
          x: centerX - clusterRadius * 0.6,
          y: centerY + clusterRadius,
          color: 0x9ca3af,
          size: 10,
          glow: 0x6b7280,
        },
      ...
      clusters.forEach((cluster) => {
        ...
        clusterFixtures.forEach((fixture, index) => {
          ...
          const x =
            cluster.x + Math.cos(angle) * spread + (Math.random() - 0.5) * 30;
          const y =
            cluster.y + Math.sin(angle) * spread + (Math.random() - 0.5) * 30;
      ...
      clusters.forEach((cluster) => {
        ...
        label.y =
          cluster.y -
          (Math.min(180, 60 + fixturesByQuality[cluster.grade].length * 3) +
            60);
        label.alpha = 0.8;
  ...
  useEffect(() => {
    const handleResize = () => {
      if (appRef.current && containerRef.current) {
        appRef.current.renderer.resize(containerRef.current.clientWidth, 700);
      }
    };
  ...
  return (
    <div className="relative">
      <div
        ref={containerRef}
        className="w-full rounded-3xl overflow-hidden border-2 border-purple-500/30 shadow-2xl shadow-purple-500/10"
      />
        ...
        <div className="absolute top-4 left-4 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 backdrop-blur-xl border-2 border-purple-500/50 rounded-2xl p-5 max-w-md pointer-events-none shadow-2xl shadow-purple-500/20">
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <span className="px-2 py-1 text-xs font-bold bg-purple-500/20 text-purple-300 rounded-lg border border-purple-500/30">
                {hoveredFixture.quality_scores?.find(
                  (q) => q.market_key === "match_winner",
                )?.final_grade || "N/A"}
              </span>
          ...
          <div className="mb-4 pb-4 border-b border-gray-700">
            <div className="flex items-center justify-between mb-1">
              ...
              {hoveredFixture.predictions?.find(
                (p) => p.market_key === "match_winner",
              ) && (
                <div className="text-sm font-semibold text-green-400">
                  {(() => {
                    const pred = hoveredFixture.predictions.find(
                      (p) => p.market_key === "match_winner",
                    );
                    if (pred && typeof pred.prediction === "object") {
                      return (
                        ((pred.prediction as any).home_win * 100).toFixed(0) +
                        "%"
                      );
                    }
                    return "";
              ...
              {hoveredFixture.predictions?.find(
                ...
                <div className="text-sm font-semibold text-red-400">
                  {(() => {
                    ...
                    if (pred && typeof pred.prediction === "object") {
                      return (
                        ((pred.prediction as any).away_win * 100).toFixed(0) +
                        "%"
                      );
                    }
                    return "";
          ...
          {hoveredFixture.predictions &&
          hoveredFixture.predictions.length > 0 ? (
            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
              <div className="text-xs font-bold text-purple-300 mb-2 flex items-center gap-2">
                <span>🎯</span> AI PREDICTIONS (
                {hoveredFixture.predictions.length} markets)
              </div>

              {(() => {
                // Organize predictions by category
                const categories = {
                  "⚽ GOALS": hoveredFixture.predictions.filter(
                    (p) =>
                      (p.market_key.includes("over_under") &&
                        !p.market_key.includes("corners") &&
                        !p.market_key.includes("cards") &&
                        !p.market_key.includes("shots") &&
                        !p.market_key.includes("offsides") &&
                        !p.market_key.includes("first_half")) ||
                      p.market_key.includes("team_over") ||
                      p.market_key === "match_winner" ||
                      p.market_key === "both_teams_score",
                  ),
                  "🚩 CORNERS": hoveredFixture.predictions.filter((p) =>
                    p.market_key.includes("corners"),
                  ),
                  "🟨 CARDS": hoveredFixture.predictions.filter((p) =>
                    p.market_key.includes("cards"),
                  ),
                  "🎯 SHOTS": hoveredFixture.predictions.filter((p) =>
                    p.market_key.includes("shots"),
                  ),
                  "⚡ OFFSIDES": hoveredFixture.predictions.filter((p) =>
                    p.market_key.includes("offsides"),
                  ),
                  "⏱️ FIRST HALF": hoveredFixture.predictions.filter((p) =>
                    p.market_key.includes("first_half"),
                  ),
                };
                ...
                return Object.entries(categories).map(
                  ([categoryName, predictions]) => {
                    ...
                    return (
                      ...
                      >
                        <div className="text-[10px] font-bold text-purple-400 mb-1.5">
                          {categoryName}
                        </div>
                        <div className="space-y-1.5">
                          {topPredictions.map((pred, idx) => {
                            const quality = hoveredFixture.quality_scores?.find(
                              (q) => q.market_key === pred.market_key,
                            );
                            return (
                              <div
                                key={idx}
                                className="bg-gray-800/30 rounded-md p-1.5 hover:bg-gray-800/50 transition-colors"
                              >
                                <div className="flex items-center justify-between mb-0.5">
                                  <span className="text-[10px] font-medium text-gray-300">
                                    {pred.market_key
                                      .replace(/_/g, " ")
                                      .replace("over under", "O/U")
                                      .toUpperCase()}
                                  </span>
                                  {quality && (
                                    <span
                                      className={`text-[9px] font-bold px-1 py-0.5 rounded ${
                                        quality.final_grade === "A"
                                          ? "bg-green-500/30 text-green-300"
                                          : quality.final_grade === "B"
                                            ? "bg-amber-500/30 text-amber-300"
                                            : "bg-gray-500/20 text-gray-400"
                                      }`}
                                    >
                                      {quality.final_grade}{" "}
                                      {(quality.model_confidence * 100).toFixed(
                                        0,
                                      )}
                                      %
                                    </span>
                                  ...
                                  {(() => {
                                    if (typeof pred.prediction === "object") {
                                      ...
                                      if (pred.market_key === "match_winner") {
                                        ...
                                        pred.market_key === "both_teams_score"
                                      ) {
              ...
              <div className="text-[10px] text-gray-500 text-center pt-2 border-t border-gray-800">
                💡 Showing Grade A/B picks only • Click fixture for all{" "}
                {hoveredFixture.predictions.length} markets
              </div>
            </div>
          ) : (
