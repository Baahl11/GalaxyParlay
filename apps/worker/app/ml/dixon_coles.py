...
ENHANCED (v3.0) - FASE 5 League-Specific Calibration:
8. League-specific home advantage values
9. Champions League: +0.08 home advantage boost (63.89% → 70%+ target)
10. Bundesliga: +0.08 home advantage boost (68.50% → 72%+ target)
...
try:
    ...
    logger_temp.warning("FIFA scraper not available, predictions will use base model only")
...
class DixonColesModel:
    ...
    def __init__(
        ...
        # FIFA integration flag
        self.use_fifa = FIFA_AVAILABLE

        # Try to load cached model on init
        self._load_from_cache()

        # Sync team names with multi_market_predictor if loaded
        if self._is_fitted:
            self._sync_team_names_to_multi_market()

    def _sync_team_names_to_multi_market(self) -> None:
        """
        Sync team names to multi_market_predictor for FIFA integration.

        CRITICAL: Without this, FIFA returns None because multi_market_predictor
        doesn't have team names to look up FIFA ratings.
        """
        try:
            from .multi_market_predictor import multi_market_predictor
            ...
            if self.team_names:
                logger.info(
                    "Team names synced to multi_market_predictor for FIFA",
                    count=len(self.team_names),
                )
        except Exception as e:
            logger.warning("Could not sync team names to multi_market_predictor", error=str(e))
    ...
    def _get_fifa_adjustments(
        self, home_team_id: int, away_team_id: int
    ) -> Optional[Dict[str, float]]:
        """
        Get FIFA-based adjustments for match prediction.
        ...
        - pace_advantage: Pace rating difference

        Returns None if FIFA not available or teams not found.
        """
        if not self.use_fifa:
            return None

        try:
            # Get team names from ID mapping
    ...
    def _load_from_cache(self) -> bool:
        ...
        try:
            ...
            logger.info(
                "Dixon-Coles model loaded from cache",
    ...
    def tau(
        self, home_goals: int, away_goals: int, lambda_home: float, mu_away: float, rho: float
    ) -> float:
        """
    ...
    def time_weight(self, match_date: datetime, current_date: Optional[datetime] = None) -> float:
        """
    ...
    def fit(self, fixtures: List[Dict[str, Any]], min_matches: int = 5) -> "DixonColesModel":
        """
        ...
        if len(valid_fixtures) < 20:
            logger.warning(
                "Not enough fixtures to fit Dixon-Coles model", count=len(valid_fixtures)
            )
            return self
        ...
        for iteration in range(15):
            ...
            for f in valid_fixtures:
                ...
                exp_home = avg_goals * math.exp(self.home_advantage + attack[ht] + defense[at])
                exp_away = avg_goals * math.exp(-self.home_advantage + attack[at] + defense[ht])
        ...
        low_score_matches = [
            f for f in valid_fixtures if f["home_score"] <= 1 and f["away_score"] <= 1
        ]
        if len(low_score_matches) > 100:
            # Count 0-0 and 1-1 draws vs 1-0 and 0-1
            draws_00 = sum(
                1 for f in low_score_matches if f["home_score"] == 0 and f["away_score"] == 0
            )
            draws_11 = sum(
                1 for f in low_score_matches if f["home_score"] == 1 and f["away_score"] == 1
            )
            wins_10 = sum(
                1 for f in low_score_matches if f["home_score"] == 1 and f["away_score"] == 0
            )
            wins_01 = sum(
                1 for f in low_score_matches if f["home_score"] == 0 and f["away_score"] == 1
            )
    ...
    def predict_score_probs(self, home_team_id: int, away_team_id: int) -> np.ndarray:
        """
    ...
    def predict_match(
        self, home_team_id: int, away_team_id: int, league_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        if league_id:
            league_home_adv, _ = get_league_home_advantage(league_id)
            effective_home_adv = league_home_adv
        else:
            effective_home_adv = self.home_advantage

        # Apply European competition reduction if needed
        if is_european:
        ...
        # NEW: FIFA quality advantage adjustments
        fifa_adjustments = self._get_fifa_adjustments(home_team_id, away_team_id)
        if fifa_adjustments:
            quality_gap = fifa_adjustments["quality_advantage"]
            star_gap = fifa_adjustments["star_players_gap"]

            # Strong favorite detection (quality gap > 5)
            if quality_gap > 5:
                boost_factor = min(0.15, quality_gap * 0.02)  # Max 15% boost
                transfer = boost_factor * draw  # Steal from draw
                home_win += transfer
                draw -= transfer
                logger.debug(
                    "fifa_quality_boost",
                    quality_gap=quality_gap,
                    boost=f"+{boost_factor*100:.1f}% home win",
                )
            elif quality_gap < -5:
                boost_factor = min(0.15, abs(quality_gap) * 0.02)
                transfer = boost_factor * draw
                away_win += transfer
                draw -= transfer
                logger.debug(
                    "fifa_quality_boost",
                    quality_gap=quality_gap,
                    boost=f"+{boost_factor*100:.1f}% away win",
                )

            # Star player advantage (3+ star players difference)
            if star_gap > 3:
                star_boost = 0.05  # +5% to favorite
                home_win += star_boost
                away_win -= star_boost * 0.5
                draw -= star_boost * 0.5
                logger.debug("fifa_star_boost", star_gap=star_gap, boost="+5% home win")
            elif star_gap < -3:
                star_boost = 0.05
                away_win += star_boost
                home_win -= star_boost * 0.5
                draw -= star_boost * 0.5
                logger.debug("fifa_star_boost", star_gap=star_gap, boost="+5% away win")
        ...
        return {
            ...
            "over_under_2_5": {"over": round(over_2_5, 4), "under": round(1 - over_2_5, 4)},
            "over_under_3_5": {"over": round(over_3_5, 4), "under": round(1 - over_3_5, 4)},
    ...
    def _predict_score_probs_adjusted(
        self, home_team_id: int, away_team_id: int, effective_home_adv: float
    ) -> np.ndarray:
        """
