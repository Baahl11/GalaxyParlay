...
MARKETS = {
    ...
    "over_under_2.5": {"outcomes": ["over", "under"], "description": "Total Goals Over/Under 2.5"},
    "both_teams_score": {"outcomes": ["yes", "no"], "description": "Both Teams to Score"},
    # Additional goal lines
    "over_under_1.5": {"outcomes": ["over", "under"], "description": "Total Goals Over/Under 1.5"},
    "over_under_3.5": {"outcomes": ["over", "under"], "description": "Total Goals Over/Under 3.5"},
    # Corners
...
class MatchPredictor:
    ...
    def load_elo_from_db(self, season: int = 2025):
        ...
        try:
            ...
            if elo_records:
                ...
                logger.info("elo_loaded_from_db", teams_loaded=len(elo_records), season=season)
    ...
    def load_historical_stats(self):
        ...
        try:
            ...
            if fixtures:
                ...
                logger.info(
                    "historical_stats_loaded", teams=len(stats), fixtures_analyzed=len(fixtures)
                )
                return len(stats)
    ...
    def predict_fixture(
        ...
        use_live_xg: bool = True,  # NEW: Fetch xG from API when available
    ) -> List[Dict[str, Any]]:
        """
        Generate predictions for a fixture

        ENHANCED v3.0: Integrates Expected Goals (xG) from API-Football

        Args:
            fixture: Fixture data from database
            include_all_markets: Whether to predict all markets or just match_winner
            referee_name: Name of referee for this match (for cards predictions)
            use_live_xg: Fetch real xG from API (for finished matches or bookmaker xG)
        ...
        # Extract referee from fixture if not provided
        if not referee_name and fixture.get("referee"):
            referee_name = fixture["referee"]

        # Detect if this is a derby (same city teams)
        # For now, simple heuristic - improve later with team metadata
        is_derby = False

        # Detect match importance
        # Cup matches, top 6 vs top 6, relegation battles, etc.
        match_importance = "normal"
        if fixture.get("round", "").lower() in ["final", "semi-final", "quarter-final"]:
            match_importance = "high"

        # Get Elo-based prediction
        elo_pred = self.elo.predict_match(home_id, away_id, league_id)

        # Calculate Expected Goals - PRIORITIZE REAL xG DATA
        home_xg = elo_pred.get("home_expected_goals", 1.5)
        away_xg = elo_pred.get("away_expected_goals", 1.2)
        xg_source = "elo_estimate"

        # NEW: Try to fetch real xG from API-Football
        if use_live_xg:
            try:
                from ..services.apifootball import api_football_client

                xg_data = api_football_client.get_fixture_xg(fixture_id)

                # Use real xG if available (much better signal than estimated)
                if xg_data.get("home_xg") is not None and xg_data.get("away_xg") is not None:
                    home_xg = xg_data["home_xg"]
                    away_xg = xg_data["away_xg"]
                    xg_source = "api_football_actual"

                    logger.info(
                        "Using real xG data from API-Football",
                ...
                logger.warning(
                    "Could not fetch xG from API, using estimates",
                    fixture_id=fixture_id,
                    error=str(e),
                )
        ...
        if include_all_markets:
            ...
            if referee_name:
                try:
                    ...
                    if referee_data:
                        logger.info(
                            "Using referee profile for cards prediction",
                            referee=referee_name,
                            avg_cards=referee_data.get("avg_yellow_cards"),
                        )
                except Exception as e:
                    logger.warning(
                        "Could not fetch referee data, using defaults",
                        referee=referee_name,
                        error=str(e),
                    )

            # Set team names for FIFA integration (CRITICAL FOR MAXIMIZING FIFA USAGE)
                    ...
                    if "total_over" in corner_key:
                        ...
                        predictions.append(
                            self._format_prediction(
                                ...
                                confidence=confidence,
                                features_used={
                                    "expected_corners": corners.get("expected", {}).get(
                                        "total", 10.5
                                    )
                                },
                            )
                    ...
                    if "total_over" in card_key:
                        ...
                        predictions.append(
                            self._format_prediction(
                                ...
                                confidence=confidence,
                                features_used={
                                    "expected_cards": cards.get("expected", {}).get(
                                        "total_yellow", 3.5
                                    )
                                },
                            )
            ...
            for shot_key, data in shots.items():
                if isinstance(data, dict) and "over" in data and "under" in data:
                    if "sot_over" in shot_key:
                        market_key = (
                            f"shots_on_target_over_under_{shot_key.replace('sot_over_', '')}"
                        )
                        ...
                        predictions.append(
                            self._format_prediction(
                                ...
                                confidence=confidence,
                                features_used={
                                    "expected_sot": shots.get("expected", {}).get(
                    ...
                    if "total_over" in offside_key:
                        ...
                        predictions.append(
                            self._format_prediction(
                                ...
                                confidence=confidence,
                                features_used={
                                    "expected_offsides": offsides.get("expected", {}).get(
                                        "total", 4.5
                                    )
                                },
                            )
                ...
                if isinstance(data, dict) and "over" in data and "under" in data:
                    ...
                    predictions.append(
                        self._format_prediction(
                            ...
                            confidence=confidence,
                            features_used={
                                "expected_goals": home_xg if team == "home" else away_xg
                            },
                        )
                    )
            ...
            if half_time:
                ...
                confidence = self._calculate_market_confidence(
                    max(first_half_over, first_half_under)
                )
                ...
                predictions.append(
                    self._format_prediction(
                        ...
                        prediction={
                            "over": round(first_half_over, 4),
                            "under": round(first_half_under, 4),
                        },
                        confidence=confidence,
    ...
    def _predict_btts(self, elo_pred: Dict[str, float], fixture: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "probabilities": {"yes": round(btts_prob, 3), "no": round(no_btts_prob, 3)},
            "confidence": round(min(0.85, confidence), 2),
            "features": {"hist_btts": hist_btts, "elo_btts": round(elo_btts, 3)},
        }

    def _calculate_confidence(self, max_prob: float, elo_diff: float) -> float:
        """
    ...
    def batch_predict(self, fixtures: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for fixture in fixtures:
            try:
                ...
                logger.error("prediction_failed", fixture_id=fixture.get("id"), error=str(e))
                continue
