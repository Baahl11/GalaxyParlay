...
async def test_player_props_debug():
    ...
    return {
        ...
        "method_result_home_players": [
            p.get("player_name") for p in result.get("home_players", [])[:3]
        ],
        "method_result_away_players": [
            p.get("player_name") for p in result.get("away_players", [])[:3]
        ],
        "full_result": result,
    }
