...
from app.services.apifootball import APIFootballClient, transform_fixture_to_db, transform_odds_to_db
from app.services.database import db_service
from app.ml import MatchPredictor, QualityScorer
from app.ml.elo import EloRatingSystem, DEFAULT_RATINGS, TOP_TEAM_BONUSES
...
def get_team_historical_stats(
    ...
    try:
        ...
        if not fixtures:
            return {
                "status": "warning",
                "message": "No historical fixtures found",
                "stats": []
            }
        ...
        return {
            ...
            "predictions": {
                ...
                "expected_goals": {
                    ...
                    "total": round(features.get("expected_home_goals", 1.5) + features.get("expected_away_goals", 1.0), 2)
                }
            },
            "elo_analysis": {
...
def sync_all_historical_data(
    season: int = 2024,
    matches_per_league: int = 150,
    recalculate_elo: bool = True
):
    """
...
def _recalculate_all_elo() -> Dict[str, Any]:
    ...
    for fixture in all_finished:
        ...
        # Update Elo
        elo.update_rating(home_id, away_id, home_actual, home_score - away_score, True, 1.0, league_id)
        elo.update_rating(away_id, home_id, away_actual, away_score - home_score, False, 1.0, league_id)
        ...
        for team_id, team_name, result, gf, ga in [
            ...
            if team_id not in team_stats:
                team_stats[team_id] = {
                    ...
                    "peak_rating": 1500.0, "peak_date": None,
            ...
            if result == "W": team_stats[team_id]["wins"] += 1
            elif result == "D": team_stats[team_id]["draws"] += 1
            else: team_stats[team_id]["losses"] += 1
...
def predict_with_dixon_coles(fixture_id: int):
    ...
    try:
        if not dixon_coles_model.is_fitted:
            raise HTTPException(
                status_code=400, 
                detail="Model not fitted. Call POST /jobs/fit-dixon-coles first"
            )
...
def validate_predictions(
    ...
    try:
        ...
        query = db_service.client.table("fixtures")\
            .select("*")\
            .eq("status", "FT")
        ...
        for fixture in finished_fixtures:
            ...
            for pred in predictions:
                ...
                if market == "match_winner":
                    ...
                    actual_probs = [1 if actual_result == "home_win" else 0,
        ...
        return {
            ...
            "summary": {
                ...
                "brier_interpretation": "Good" if avg_brier < 0.25 else "Fair" if avg_brier < 0.33 else "Poor"
            },
            "value_bets": {
...
def sync_team_stats(
    limit: int = Query(20, description="Max teams to sync per call")
):
    """
        ...
        for team_id, league_id in list(team_data)[:limit]:
            try:
                ...
                if stats:
                    ...
                    }, on_conflict="team_id,league_id,season").execute()
                    
                    stats_synced += 1
                    logger.info("synced_team", team_id=team_id, 
                               goals_avg=goals_scored_avg, conceded=goals_conceded_avg)
...
from app.ml.multi_market_predictor import multi_market_predictor, TeamStats
...
def get_multi_market_prediction(fixture_id: int):
    ...
    try:
        ...
        try:
            home_stats_result = db_service.client.table("team_statistics")\
                .select("stats_data")\
                .eq("team_id", home_team_id)\
                .execute()
            
            away_stats_result = db_service.client.table("team_statistics")\
                .select("stats_data")\
            ...
            if home_stats_result.data:
                multi_market_predictor.set_team_stats(
                    home_team_id, 
                    TeamStats(home_stats_result.data[0].get('stats_data'))
                )
            
            if away_stats_result.data:
                multi_market_predictor.set_team_stats(
                    away_team_id,
                    TeamStats(away_stats_result.data[0].get('stats_data'))
                )
        except Exception as e:
        ...
        is_cup = league_id in {2, 3, 848}  # UCL, UEL, UECL
...
def sync_league_team_statistics(
    league_id: int,
    season: int = Query(2025, description="Season (e.g., 2025)")
):
    """
        ...
        for team_id in list(team_ids)[:30]:  # Limit to 30 teams per call
            try:
                ...
                if stats:
                    ...
                    synced += 1
                    logger.info("Synced team stats", team_id=team_id, 
                               goals_avg=goals_scored_avg, conceded_avg=goals_conceded_avg)
        ...
        for player in players:
            try:
                ...
                db_service.client.table("player_statistics").upsert({
                    ...
                    "stats_data": player,
                    "updated_at": datetime.utcnow().isoformat()
                }, on_conflict="player_id,league_id,season").execute()
...
def sync_all_player_statistics(
    ...
    try:
        ...
        fixtures = db_service.client.table("fixtures").select(
            "home_team_id, away_team_id, league_id"
        ).eq("status", "NS").execute()
...
def get_top_scorers(
    ...
    """
    try:
        result = db_service.client.table("player_statistics").select(
            "player_name, team_id, goals, assists, total_shots, shots_on_target, goals_per_90, games_played, minutes_played"
        ).gte("goals", min_goals).order("goals", desc=True).limit(limit).execute()
...
def get_top_shooters(
    ...
    try:
        ...
        ).gte("total_shots", min_shots).order("shots_on_target", desc=True).limit(limit).execute()
